...
// Writer writes T, it is intended to act as a generic variant of io.Writer.
...
// Writer is returned; if 'f' is nil, the encoder is set to gob.NewEncoder.
// Example:
//
...
func NewWriterFromValues[T any](w io.Writer) func(f encoderFn) Writer[T] {
	return func(f func(io.Writer) Encoder) Writer[T] {
		...
		b := bytes.NewBuffer(nil)
		e := func(w io.Writer) Encoder { return gob.NewEncoder(w) }(b)
...
// io.Writer is returned; if 'f' is nil, the decoder is set to gob.NewDecoder.
// Example:
//
...
func NewWriterFromBytes[T any](w Writer[T]) func(f decoderFn) io.Writer {
	return func(f decoderFn) io.Writer {
		...
		b := bytes.NewBuffer(nil)
		d := func(r io.Reader) Decoder { return gob.NewDecoder(r) }(b)
